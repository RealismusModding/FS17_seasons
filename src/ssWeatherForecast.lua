...
function ssWeatherForecast:update(dt)
    if g_seasons.showControlsInHelpScreen then
        if not self.hud.visible then
            ...
            g_currentMission:addHelpButtonText(g_i18n:getText("SEASONS_HIDE_WF"), InputBinding.SEASONS_SHOW_WF)
        end
    end
...
function ssWeatherForecast:drawForecast(forecast)
    ...
    for n = 2, ssWeatherManager.forecastLength do
        ...
        -- Render Season Days
        setTextAlignment(RenderText.ALIGN_CENTER)
        renderText(self.hud.posX + posXOffset + self.hud.iconWidth + self.hud.iconWidthSmall*3/4 + dayOffset, self.hud.posY + self.hud.height - posYOffset - self.hud.iconHeight, self.hud.textSize*1.2, tostring(ssSeasonsUtil:dayInSeason(forecast[n].day)))
        ...
        renderText(self.hud.posX + posXOffset + dayOffset, self.hud.posY + self.hud.height - posYOffset - self.hud.iconHeightSmall/2, self.hud.textSize*1.2, ssSeasonsUtil:dayNameShort(ssSeasonsUtil:dayOfWeek()+n-1))
    end
end
...
function ssWeatherForecast:drawToday(forecast)
    ...
    renderText(self.hud.clockPosX + self.hud.clockWidth/2,self.hud.clockPosY, self.hud.textSize*1.5, string.format("%02d/%s/%d", ssSeasonsUtil:dayInSeason(forecast[1].day), ssSeasonsUtil.seasons[forecast[1].season], ssSeasonsUtil:year() + 2017))
