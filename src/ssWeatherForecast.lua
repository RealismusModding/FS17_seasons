...
function ssWeatherForecast:loadMap(name)
    if g_currentMission:getIsClient() then
        ...
        self.hud.overlays.seasons[ssSeasonsUtil.SEASON_SPRING] = Overlay:new("hud_spring", Utils.getFilename("resources/huds/hud_Season_Color/hud_spring_Color.png", ssSeasonsMod.modDir), 0, 0, width, height)
        self.hud.overlays.seasons[ssSeasonsUtil.SEASON_SUMMER] = Overlay:new("hud_summer", Utils.getFilename("resources/huds/hud_Season_Color/hud_summer_Color.png", ssSeasonsMod.modDir), 0, 0, width, height)
        self.hud.overlays.seasons[ssSeasonsUtil.SEASON_AUTUMN] = Overlay:new("hud_autumn", Utils.getFilename("resources/huds/hud_Season_Color/hud_autumn_Color.png", ssSeasonsMod.modDir), 0, 0, width, height)
        self.hud.overlays.seasons[ssSeasonsUtil.SEASON_WINTER] = Overlay:new("hud_winter", Utils.getFilename("resources/huds/hud_Season_Color/hud_winter_Color.png", ssSeasonsMod.modDir), 0, 0, width, height)
...
function ssWeatherForecast:draw()
    ...
    if self.hud.visible then
        ...
        for n = 2, ssWeatherManager.forecastLength do
            ...
            -- Render Season Icon
            renderOverlay(self.hud.overlays.seasons[forecast[n].season].overlayId, WeatherForecastPosX + 0.086 + (daysPosOffset * (n - 2)), WeatherForecastPosY + 0.074, 0.0185, 0.0335)
    ...
    -- Render Season Icon
    renderOverlay(self.hud.overlays.seasons[forecast[1].season].overlayId, dayPosX + 0.009 * uiScale, dayPosY + 0.033 * uiScale, 0.0185 * uiScale, 0.0335 * uiScale)
