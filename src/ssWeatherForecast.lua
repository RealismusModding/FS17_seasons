...
function ssWeatherForecast:draw()
    ...
    if self.hud.visible then
		...
		-- Get timeBgOverlay position offset WeatherForecast overlay.
		local WeatherForecastPosX = g_currentMission.timeBgOverlay.x + self.hud.posX;
		local WeatherForecastPosY = g_currentMission.timeBgOverlay.y + self.hud.posY;
