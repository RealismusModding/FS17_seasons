...
ssSoilCompaction.superFunc = {} -- To store function pointers in Utils that we intend to overwrite
ssSoilCompaction.cultivatorDecompactionDelta = 1 -- Cultivators additive effect on the compaction layer

ssSoilCompaction.overlayColor = {} -- Additional colors for the compaction overlay (false/true: useColorblindMode)
ssSoilCompaction.overlayColor[false] = {
    {0.6172, 0.0510, 0.0510, 1},
    {0.6400, 0.1710, 0.1710, 1},
    {0.6672, 0.3333, 0.3333, 1},
}

ssSoilCompaction.overlayColor[true] = {
    {0.6172, 0.0510, 0.0510, 1},
    {0.6400, 0.1710, 0.1710, 1},
    {0.6672, 0.3333, 0.3333, 1},
}

function ssSoilCompaction:preLoad()
    g_seasons.soilCompaction = self
end

function ssSoilCompaction:load(savegame, key)
    self.compactionEnabled = ssXMLUtil.getBool(savegame, key .. ".settings.soilCompactionEnabled", true)
end

function ssSoilCompaction:save(savegame, key)
    ssXMLUtil.setBool(savegame, key .. ".settings.soilCompactionEnabled", self.compactionEnabled)
end

function ssSoilCompaction:loadMap()
    -- Overwritten functions
    ssUtil.overwrittenFunction(InGameMenu, "generateFruitOverlay", ssSoilCompaction.generateFruitOverlay)

    ssUtil.overwrittenStaticFunction(Utils, "cutFruitArea", ssSoilCompaction.cutFruitArea)
    ssUtil.overwrittenStaticFunction(Utils, "updateCultivatorArea", ssSoilCompaction.updateCultivatorArea)
end

function ssSoilCompaction:readStream(streamId, connection)
    self.compactionEnabled = streamReadBool(streamId)
end

function ssSoilCompaction:writeStream(streamId, connection)
    streamWriteBool(streamId, self.compactionEnabled)
end

function ssSoilCompaction.cutFruitArea(superFunc, fruitId, startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ, destroySpray, destroySeedingWidth, useMinForageState)
...
function ssSoilCompaction.updateCultivatorArea(superFunc, x, z, x1, z1, x2, z2, ...)
    ...
    addDensityMaskedParallelogram(
        ...
        ssSoilCompaction.cultivatorDecompactionDelta
    )
...
function ssSoilCompaction:generateFruitOverlay(superFunc)
    ...
    if self.mapNeedsPlowing and self.mapSelectorMapping[self.mapOverviewSelector:getState()] == InGameMenu.MAP_SOIL then
        if g_currentMission ~= nil and g_currentMission.terrainDetailId ~= 0 then
            ...
            local colors = ssSoilCompaction.overlayColor[g_gameSettings:getValue("useColorblindMode")]
